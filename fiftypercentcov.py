...
def generate_narrow_search_terms(location):
    """Generate a few narrower search queries for Wikipedia."""
    return [
        f"{location} history",
...
def narrow_wikipedia_lookup_one(location) -> str:
    ...
    possible_dates = []
    # We'll remove or reduce the sleep to zero; you can re-add if you get rate-limited
    # time.sleep(0.01)
    ...
    for query_str in generate_narrow_search_terms(location):
        try:
            ...
            for title in results[:2]:  # limit to top 2 results
                try:
...
def parallel_lookup(locations, lookup_func, max_workers=10, desc="Wikipedia Lookup"):
    ...
    # Wrap the single lookup in a function so we can return (loc, date_str)
    def run_one(loc):
...
# -------------- PIPELINE --------------
def main():
    input_file = "haunted_places_initial.tsv"
    df = pd.read_csv(input_file, sep="\t")

    # Step A: Local date extraction with progress bar
    logging.info("Extracting local text dates...")
    df["LocalDate"] = df["description"].progress_apply(extract_date_from_text)

    # Step B: Identify rows that need Wikipedia (default date)
    logging.info("Determining missing rows for Wikipedia lookup...")
    mask_missing = (df["LocalDate"] == DEFAULT_DATE)
    missing_locs = df.loc[mask_missing, "location"].dropna().unique()

    # Optionally skip single-word/short locations
    if SKIP_SHORT_LOCATIONS:
        missing_locs = [loc for loc in missing_locs if not should_skip_location(loc)]

    # Sample a fraction of missing locations if we only want partial coverage
    if SAMPLE_MISSING_FRAC < 1.0:
        missing_locs_series = pd.Series(missing_locs)
        # sample some fraction
    ...
    logging.info(
        ...
        f"we will look up {len(missing_locs_sampled)} of them in narrow Wikipedia."
    )

    # Step C: Narrow Wikipedia lookups in parallel
    ...
    # Step D: Broad approach for remaining (optional)
    if USE_BROADER_APPROACH:
        logging.info("Broad Wikipedia approach for any still missing...")
        mask_still_missing = (df["NarrowWikiDate"] == DEFAULT_DATE)
        missing_locs2 = df.loc[mask_still_missing, "location"].dropna().unique()

        # Possibly skip short locations again
        if SKIP_SHORT_LOCATIONS:
            missing_locs2 = [loc for loc in missing_locs2 if not should_skip_location(loc)]

        # You can choose to sample again or skip if coverage is enough
        if SAMPLE_MISSING_FRAC < 1.0:
            # example: sample half of what's left
        ...
        logging.info(
            f"Now have {len(missing_locs2)} still missing. "
    ...
    logging.info(f"Recognized date rows (not default): {recognized_count} "
                 f"({recognized_count / total * 100:.2f}%)")
    ...
    # Save final
    output_file = "haunted_places_final_parallel.tsv"
